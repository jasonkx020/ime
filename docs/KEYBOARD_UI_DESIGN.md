...
| AI 辅助 | ✓ | FeatureBar 仅「AI」总入口 → AiAssistPanel（模式 + 关系/意图点选）；BYOK |
...
- **CandBar 保持可见**：展示识别候选；**选词后离线词表联想**（后缀 + 端侧字 bigram 重排）；隐藏拼音 composing 行
- 高度由 **正方形书写区** 自适应（边长 ≤320dp + 顶/底栏）；底栏「返回键盘」恢复 `layout_26_pinyin`
